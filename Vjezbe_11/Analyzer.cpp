...
{
  // P(r;N,p) is a probability of finding exactly r successes in N trials, when
  // probability of success in each single trial is a constant, p
  return TMath::Binomial(N, r) * pow(p, r) * pow((1 - p), N - r);
  // TMath::Binomial je povrh
}

pair<double, double> Analyzer::CPinterval(double CL, int N, int m)
{
  double p_dn = 0, p_up = 1.0, sum_up = 0, sum_dn = 0, increment = 0.001;
  int r;
...
void Analyzer::CPbelt(int N, double CL)
{
  ...
  TH1F *Histo_up = new TH1F("up", "up", 11, 0, 11); // 	Int_t 	nbinsx, const Double_t * 	xbins
  // A histogram displays numerical data by grouping data into "bins" of equal width
  TH1F *Histo_down = new TH1F("down", "down", 11, 0, 11);
  TCanvas *canvas = new TCanvas("c1", "Clopper-Pearson belt", 900, 700); // x, y
  for (int m = 0; m <= 10; m++)
  {
    Histo_up->SetBinContent(m + 1, CPinterval(CL, N, m).second - CPinterval(CL, N, m).first); // dakle širina intervala
...
void Analyzer::Game()
{
  ...
  for (j = 1; j <= 1000; j++)
  {
    ...
    if (1. / 6 <= CPinterval(0.6827, 10, r).second && 1. / 6 >= CPinterval(0.6827, 10, r).first)
      covered++;
  }
  cout << "Clopper-Pearson coverage:" << covered * 1. / 1000 << endl;
}