...
void Analyzer::MojaFunkcija()
{
  ...
  ChiSquare_function = new TF1("Chi-Square Function", "TMath::Power(([0]-[1]*x)/([2]),2)+TMath::Power(([3]-[4]*x)/([5]),2)+TMath::Power(([6]-[7]*x)/([8]),2)+TMath::Power(([9]-[10]*x)/([11]),2)+TMath::Power(([12]-[13]*x)/([14]),2)", 9.4, 11.0);
  ...
  c->SaveAs("ChiSquareFit.png");

  // canvas->SaveAs("kraf.png");
  delete c, gr;
}