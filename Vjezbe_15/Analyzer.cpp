...
void Analyzer::ObservedPValueScan()
{
  ...
  for (int i = 10; i < 690; i += 5)
  {
    ...
    for (int j = 0; j < 100000; j++)
    {
      if (r3->Rndm() > a_SM * TMath::Power((m_H + b_SM), 2) + c_SM)
        toy_histo->Fill(r3->Exp(100)); // Generate SM process for probability given by theory
      else
        toy_histo->Fill(r3->Gaus(m_H, sigma_SM * m_H)); // otherwise generate Higgs boson according to theoretical PDF
    }
